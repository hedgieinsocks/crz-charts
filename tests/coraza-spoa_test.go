...
func TestSpoaDeploymentCustom(t *testing.T) {
	...
	options := &helm.Options{
		SetValues: map[string]string{
			...
			"metrics.port":              "9200",
			"volumes[0].name":           "custom",
			"volumes[0].configMap.name": "custom",
			"volumeMounts[0].name":      "custom",
			"volumeMounts[0].mountPath": "/etc/custom",
		},
	}
	...
	Expect(deployment.Spec.Template.Spec.Containers[0].Ports[1].Protocol).To(Equal(v1.Protocol("TCP")))
	Expect(len(deployment.Spec.Template.Spec.Containers[0].Ports)).To(Equal(2))
	Expect(deployment.Spec.Template.Spec.Containers[0].VolumeMounts[1].Name).To(Equal("custom"))
	Expect(deployment.Spec.Template.Spec.Containers[0].VolumeMounts[1].MountPath).To(Equal("/etc/custom"))
	Expect(deployment.Spec.Template.Spec.Volumes[1].Name).To(Equal("custom"))
	Expect(deployment.Spec.Template.Spec.Volumes[1].ConfigMap.Name).To(Equal("custom"))
}
